...
const selectedPeriod = computed(() => periods.value.find(
  (period) => period.open && period.requestType === MILITARY_LEAVE,
) || null);
...
const returnSemester = computed(() => {
  if (!targetSemester.value) return null;
  return {
    year: targetSemester.value.year + 2,
    semester: targetSemester.value.semester,
  };
});
  ...
  try {
    const response = await myAxios.get('/api/academic/leave-request-periods', {
      params: {
        ...
        size: 100,
        active: true,
        requestType: MILITARY_LEAVE,
      },
    });
    periods.value = response.data.data.items || [];
  } catch (error) {
    periods.value = [];
    await notify(error.response?.data?.message || '군휴학 접수 기간을 불러오지 못했습니다.');
  } finally {
...
const loadRequests = async (page = 1) => {
  ...
  try {
    ...
    requests.value = [];
    await notify(error.response?.data?.message || '군휴학 신청 내역을 불러오지 못했습니다.');
  } finally {
...
const createIdempotencyKey = () => {
  ...
  return `military-leave-${suffix}`;
};

const submitRequest = async () => {
  if (isSubmitting.value) return;

  if (!selectedPeriod.value || !targetSemester.value) {
    formError.value = '현재 접수 가능한 군휴학 신청 기간이 없습니다.';
    return;
  }
  ...
  try {
    ...
    resetAttachment();
    await notify('군휴학 신청이 접수되었습니다.');
    await loadRequests(1);
  } catch (error) {
    await notify(error.response?.data?.message || '군휴학 신청 중 오류가 발생했습니다.');
  } finally {
...
<template>
  <MyPageContainer title="군휴학 신청">
    <div class="military-leave-page">
      <form
        class="request-card"
      ...
      >
        <div class="form-grid">
          ...
          <label
            class="form-field"
            for="military-leave-start-period"
          >
            <span>시작 기간</span>
            <MyInput
              id="military-leave-start-period"
              class="period-input"
          ...
          <label
            class="form-field"
            for="military-leave-return-period"
          >
            <span>복학 예정</span>
            <MyInput
              id="military-leave-return-period"
              class="period-input"
      ...
      <section class="history-section">
        ...
        <div
          v-if="isLoadingRequests || requests.length === 0"
          class="history-state"
        >
          {{ isLoadingRequests ? '신청 내역을 불러오는 중입니다.' : '신청 내역이 없습니다.' }}
        </div>
        ...
        >
          <MyTable
            class="military-leave-history-table"
            :columns="columns"
          >
            <tr
...
<style>
.military-leave-history-table.table-container .my-table th {
  padding: 12px 16px;
...
.military-leave-history-table.table-container .my-table td {
  padding: 12px 16px;
  color: var(--personal-color-primary-text-navy);
  font-size: 0.75rem;
  font-weight: 400;
