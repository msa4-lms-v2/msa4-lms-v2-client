...
defineOptions({ name: 'AdminDoubleMajorManagement' });
...
const filters = ref({ periodId: '', keyword: '', departmentId: '', status: '' });
...
const loadCatalogs = async () => {
  try {
    const [periodResponse, departmentResponse] = await Promise.all([
      myAxios.get('/api/academic/catalog/double-major-periods', { params: { page: 1, size: 100 } }),
...
const load = async (pageNumber = 1) => {
  ...
  try {
    const response = await searchAcademicChangeRequests(TYPE, {
      ...
      requestPeriodId: filters.value.periodId || undefined,
    ...
    requests.value = [];
    await notify(error.response?.data?.message || '복수전공 신청 목록을 불러오지 못했습니다.');
  } finally {
...
const resetFilters = () => {
  filters.value = { periodId: '', keyword: '', departmentId: '', status: '' };
  load(1);
};
...
const applyRequest = async () => {
  ...
  const confirmed = await confirmDialog('학장 날인본으로 교체하고 복수전공을 학적에 반영하시겠습니까?');
  if (!confirmed) return;
  ...
  try {
    const response = await applyAcademicChange(
      ...
      selectedFiles.value,
      createAcademicChangeKey('double-major-application'),
    );
    selectedRequest.value = response.data.data;
    resetProcessingForm();
    await notify('복수전공 학적 반영이 완료되었습니다.');
    await load(page.value.page);
  } catch (error) {
    await notify(error.response?.data?.message || '복수전공 학적 반영 중 오류가 발생했습니다.');
  } finally {
...
const rejectRequest = async () => {
  ...
  const confirmed = await confirmDialog('학장 날인 미확인으로 이 복수전공 신청을 반려하시겠습니까?');
  if (!confirmed) return;
  ...
  try {
    const response = await rejectAcademicChangeByAdmin(
      ...
      selectedRequest.value.id,
      reason,
      createAcademicChangeKey('double-major-rejection'),
    );
    selectedRequest.value = response.data.data;
    resetProcessingForm();
    await notify('복수전공 신청을 반려했습니다.');
    await load(page.value.page);
  } catch (error) {
    await notify(error.response?.data?.message || '복수전공 반려 처리 중 오류가 발생했습니다.');
  } finally {
    isProcessing.value = false;
  }
};
...
<template>
  <MyPageContainer title="복수전공 신청 관리">
    ...
    <div class="management-grid">
      <section class="list-card">
        <div class="section-title">
          <h3>복수전공 신청 목록</h3><span>총 {{ page.totalCount }}건</span>
        </div>
