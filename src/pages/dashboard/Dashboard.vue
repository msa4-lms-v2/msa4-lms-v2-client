...
const AcademicDashboard = defineAsyncComponent(() => import('./AcademicDashboard.vue'));
</script>

<template>
  <AdminDashboard v-if="authStore.userInfo?.role === 'ADMIN'" />
  <AcademicDashboard v-else-if="['STUDENT', 'PROFESSOR'].includes(authStore.userInfo?.role)" />
</template>
