...
<template>
  <MyPageContainer title="제적 처리" subtitle="관리자가 제적 후보를 등록하고 대기 건을 확정 또는 취소합니다.">
    ...
    <section class="dismissal-layout">
      <div>
        <div class="list-heading"><div><h3>검색 결과</h3><span>총 {{ page.totalCount || 0 }}건</span></div><MyButton btn-type="button" color="admin-indigo" size="middle" content="신규 등록" @click="resetForm" /></div>
        <MyTable :columns="columns" :loading="loading" :empty="!loading && dismissals.length === 0" empty-message="조회된 제적 후보가 없습니다.">
          <tr v-for="item in dismissals" :key="item.id">
            ...
            <td class="action-cell">
              <MyButton btn-type="button" class="secondary-button" color="white" size="small" content="상세" @click="selectDismissal(item)" />
              <MyButton v-if="item.status === 'PENDING'" btn-type="button" color="admin-indigo" size="small" content="확정" @click="confirmDismissal(item)" />
            </td>
          </tr>
        </MyTable>
      ...
      <form class="dismissal-form" @submit.prevent="save">
        <div class="form-header"><div><h3>{{ isEditing ? '제적 후보 상세' : '제적 후보 등록' }}</h3><p>{{ isEditing ? '대기 상태 후보의 근거를 수정하거나 취소할 수 있습니다.' : '학생 ID와 실제 확인 근거를 입력합니다.' }}</p></div><MyButton v-if="isEditing" btn-type="button" class="secondary-button" color="white" size="middle" content="등록 모드" @click="resetForm" /></div>
        <div class="form-row">
        ...
        <div v-if="isEditing && selected?.status === 'PENDING'" class="cancel-box">
          ...
          <MyButton btn-type="button" color="red" size="middle" content="후보 취소" @click="cancelDismissal" />
        </div>
      </form>
    </section>
  </MyPageContainer>
...
.status-chip.pending { color:var(--personal-color-status-warning-text-amber); }
...
.dismissal-form { margin-top:24px; padding:20px; border:1px solid var(--personal-color-border-mist); border-radius:8px; background:var(--personal-color-white); }
...
.form-group label { display:flex; justify-content:space-between; color:var(--personal-color-primary-text-navy); font-weight:700; font-size:.88rem; }
.form-group label span { color:var(--personal-color-text-secondary-steel); font-weight:600; }
textarea { width:100%; resize:vertical; border:1px solid var(--personal-color-border-mist); border-radius:4px; padding:12px; font:inherit; box-sizing:border-box; }
textarea:focus { outline:none; border-color:var(--personal-color-admin-secondary-indigo); }
.detail-box, .cancel-box { border:1px solid var(--personal-color-border-mist); border-radius:8px; padding:12px; background:var(--personal-color-bg-subtle-snow); margin-bottom:14px; }
